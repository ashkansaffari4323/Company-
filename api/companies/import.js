...
module.exports = async (req, res) => {
  try {
    ...
    const payloads = (req.body.companies || []).map(buildCompanyPayload).filter(Boolean);
    ...
    for (const [batchIndex, batch] of chunks(payloads, 50).entries()) {
      try {
        const response = await axios.post(`${APS}/hq/v1/accounts/${accountId}/companies/import`, batch, {
          headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }
        });
        success += response.data.success || 0;
        failure += response.data.failure || 0;
        results.push({ batch: batchIndex + 1, sentItems: batch.length, ...response.data });
      } catch (err) {
    ...
    res.json({ accountId, totalCompanies: payloads.length, batches: results.length, success, failure, results });
  } catch (err) {
    error(res, err);
  }
};
